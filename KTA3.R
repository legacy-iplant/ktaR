...
my_beta_eval <- function(x) {
	return(
		eval(parse(text=paste('mydata',beta_col,sep='$')))
		)
}
...
for (i in 1:length(locs)) {
	...
	return_RMSE <- append(return_RMSE, sqrt(mean((my_beta_eval(locs[[i]])-Beta)**2)))

	return_auc <- append(return_auc, roc(this_truth ~ my_P_eval(locs[[i]]))$auc)
