...
if (is.null(ret.opts$class)) {
	do_truth <- FALSE
} 
if (is.null(ret.opts$class)==FALSE) {
	do_truth <- TRUE
...
if (is.null(ret.opts$effect)==FALSE) {
	do_effect <- TRUE
	beta_file <- ret.opts$effect
	beta_col <- ret.opts$beta
}

if (is.null(ret.opts$thres)) {
	my_threshold <- ret.opts$thres
}
if (is.null(ret.opts$thres)==FALSE) {
	my_threshold <- 0.5
}
...
for (i in 1:length(locs)) {
	...
	if (do_truth) {
		myh <- HMeasure(this_truth, my_P_eval('mydata'))
...
use_these <- list()
possibles <- c('return_AUC','return_names','return_RMSE','return_MAE','return_H','return_Gini','return_AUCH','return_KS')
