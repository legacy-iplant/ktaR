...
for (i in 1:length(locs)) {
	...
	if (do_effect) {
		cat('\nPerforming effect size evaulation...')
	...
	if (do_truth) {
		cat('\nUsing severity ratio',sr,'for classification evaluation...')
		...
		if (sr2_do==TRUE) {
			cat('\nUsing severity ratio',sr2,'for classification evaulation...')
			myh <- HMeasure(this_truth, my_P_eval('mydata'), severity.ratio=sr2)
			return_H2 <- append(return_H2, myh$metrics[1])
		}
		if (sr3_do==TRUE) {
			cat('\nUsing severity ratio',sr3,'for classification evaluation...')
