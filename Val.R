...
usage <- function() {
  cat("--analysis to specify either 'GWAS' or 'prediction' (if blank, Validate assumes GWAS)\n")
  cat("--folder to input folder of box results (required)\n")
  cat("--class to specify the known-truth file for used simulation (required)\n")
  cat("--snp to specify a string for the name of the SNP column in results file (required)\n")
  cat("--score to specify a string for the name of the scoring column in results file (e.g., p-value; required)\n")
  cat("--effect to specify a file for just effects aligning with the class file (only required for OTE1 kttype)\n")
  cat("--beta to specify a string for the name of the estimated SNP effect column in results file\n")
  cat("--severity to specify a severity ratio to use in calculating the H-measure (recommended 1 or pi1/pi0)\n")
  cat("--filename to specify the desired filename for the Validate output file\n")
  cat("--threshold to specify a desired threshold for classification performetrics where necessary\n")
  cat("--seper to specify either whitespace or comma\n")
...
if (is.null(ret.opts$analysis)) {
  analysis <- "GWAS"
} else {
  analysis <- ret.opts$analysis
}
...
if (analysis=="GWAS") {
        ...
        if (is.null(ret.opts$effect) & kttype=="OTE1") {
          do.effect <- FALSE
        } else if (is.null(ret.opts$effect) & kttype!="OTE1") {
          do.effect <- TRUE
          beta.col <- ret.opts$beta
        } else if (is.null(ret.opts$effect)!=FALSE & kttype=="OTE1") {
          do.effect <- TRUE
...
if (analysis=="prediction") {
  cat("You chose prediction. Which is hilarious really, because there isn't anything for that yet.\n")
}